...
for epoch in np.arange(0, args["epochs"]):
    ...
    #'error', which is the difference between our prediction and
    #the true values
    error = preds - trainY
    loss = np.sum(error ** 2)
    losses.append(loss)
    ...
    #features and the error of the predictons
    gradient = trainX.T.dot(error)

    #in the update stage, all wa nees to do is "nudge" the weight
    #matrix in the negative direction of the gradient (hence the
    #term "gradient descent" by taking a small step towards a set
    # of "more optimal" parametars
